	...
	If $Answer = 2 Then
		...
		ElseIf $ChatState = 1 Then
			If $ChatGreeting = 0 Then
				$Greeting = " Здравствуйте, меня зовут " & StringStripWS ($OpName, 2) & ", специалист технической поддержки абонентов " & '"Триколор ТВ".' & " Уточните, пожалуйста, где установлен комплект оборудования для просмотра спутникового телевидения «Триколор ТВ», которым Вы пользуетесь в настоящее время: дома, в месте постоянного проживания; на даче; на территории заведения (кафе, спортзала, гостиницы и т. д.)?"
				$IsDataCorrect = 1
			EndIf
			If $ChatGreeting <> 0 Then
